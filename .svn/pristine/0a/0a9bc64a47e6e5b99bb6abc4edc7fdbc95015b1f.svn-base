				...
				<tbody id="radiWaitBody">
						<c:forEach items="${waitList }" var="wait">
							<tr data-rcp-no="${wait['rcpNo'] }">
								...
								<td>${wait["paName"] }</td>
								<td><select class="filmCateSelect">
										<c:forEach items="${filmCate }" var="filmCate">
											<option value="${filmCate['filmCd'] }"
												${filmCate['filmCd'] eq wait['filmCd'] ? "selected" : "" }>${filmCate["filmNm"] }</option>
										</c:forEach>
								</select></td>
								<td><select class="statusSelect">
										<c:forEach items="${waitStatus }" var="waitSt">
											<option value="${waitSt['waitstCd'] }"
												${waitSt['waitstCd'] eq wait['waitstCd'] ? "selected" : "" }>${waitSt["waitstNm"] }</option>
										</c:forEach>
								</select></td>
							</tr>
						</c:forEach>
...
<form class="row g-3">
	<div class="offcanvas offcanvas-end trm" data-bs-scroll="true"
		...
		<div class="offcanvas-body row g-3">
			...
			<div class="col-12 radiChatTable">
...
// 					});
				...
				let btnTag = $("<input>").attr({
									"type": "button"
									, "value" : "변경"
									, "class" : "btn_blue filmInputBtn"
// 									, "onclick" : "filmInput(\""+v.rcpNo+"\",\""+v.paName+"\",\""++"\",\""++"\");"
							});
				let trTag = $("<tr>").attr({
								"class":"radiWaitClass",
